...
class SearchFilter {
  ...
  const SearchFilter({this.categories = const [], this.tags = const []});
...
class SimpleSearchEngine {
  ...
  Future<List<SearchResult>> search({
    ...
    for (final item in _index.values) {
      ...
      if (query.isEmpty) {
        results.add(
          SearchResult(item: item, relevanceScore: 1.0, matchedKeywords: []),
        );
      } else {
        // クエリにマッチするかチェック
...
void main() {
  group('Search Functionality', () {
    ...
    test('should filter by category', () async {
      ...
      final results = await searchEngine.search(query: '', filter: filter);
    ...
    test('should generate autocomplete suggestions', () {
      ...
      final suggestions = searchEngine.generateAutocompleteSuggestions(
        query: 'mor',
      );
    ...
    test('should handle multiple word search', () async {
      ...
      expect(results, isNotEmpty);
      expect(
        results.first.matchedKeywords,
        containsAll(['morning', 'exercise']),
      );
    });
