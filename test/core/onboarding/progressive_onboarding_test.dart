...
void main() {
  group('Progressive Onboarding System Tests', () {
    ...
    group('ProgressiveHintService', () {
      testWidgets('shows first quest hint when not shown before', (tester) async {
          ...
          MaterialApp(
            home: Builder(
              builder: (context) {
                return Scaffold(
                  body: ElevatedButton(
                    onPressed: () => ProgressiveHintService.showFirstQuestHint(context),
      ...
      testWidgets('does not show hint when already shown', (tester) async {
        // ヒントを表示済みとしてマーク
        await ProgressiveHintService.markHintShown(ProgressiveHintService.hintFirstQuest);
      ...
      test('resets all hints correctly', () async {
        // 複数のヒントを表示済みとしてマーク
        await ProgressiveHintService.markHintShown(ProgressiveHintService.hintFirstQuest);
        await ProgressiveHintService.markHintShown(ProgressiveHintService.hintFirstCompletion);
        await ProgressiveHintService.markHintShown(ProgressiveHintService.hintStreak);

        // 全て表示済み状態を確認
        expect(await ProgressiveHintService.hasShownHint(ProgressiveHintService.hintFirstQuest), true);
        expect(await ProgressiveHintService.hasShownHint(ProgressiveHintService.hintFirstCompletion), true);
        expect(await ProgressiveHintService.hasShownHint(ProgressiveHintService.hintStreak), true);

        // 全てリセット
        await ProgressiveHintService.resetAllHints();

        // 全て未表示状態を確認
        expect(await ProgressiveHintService.hasShownHint(ProgressiveHintService.hintFirstQuest), false);
        expect(await ProgressiveHintService.hasShownHint(ProgressiveHintService.hintFirstCompletion), false);
        expect(await ProgressiveHintService.hasShownHint(ProgressiveHintService.hintStreak), false);
      });
    });
