...
class MockOfflineOperationsService extends Mock implements OfflineOperationsService {}
class MockNetworkStatusService extends Mock implements NetworkStatusService {}
class MockIsarCollection extends Mock implements IsarCollection<LocalChallenge> {}
class MockQueryBuilder extends Mock implements QueryBuilder<LocalChallenge, LocalChallenge, QQueryOperations> {}
  ...
  group('OfflineChallengeService', () {
    ...
    group('getActiveChallenges', () {
      test('returns active challenges within date range', () async {
        ...
        when(() => mockQuery.startDateLessThanOrEqualTo(any())).thenReturn(mockQuery);
        when(() => mockQuery.endDateGreaterThanOrEqualTo(any())).thenReturn(mockQuery);
        when(() => mockQuery.sortByStartDateDesc()).thenReturn(mockQuery);
        when(() => mockQuery.findAll()).thenAnswer((_) async => [activeChallenge]);
    ...
    group('updateChallengeProgress', () {
      test('updates progress successfully when challenge exists and is active', () async {
        // Arrange
        final now = DateTime.now();
        ...
        final mockQuery = MockQueryBuilder();
        when(() => mockCollection.filter()).thenReturn(mockQuery);
        when(() => mockQuery.challengeIdEqualTo('challenge-1')).thenReturn(mockQuery);
        when(() => mockQuery.findFirst()).thenAnswer((_) async => challenge);
        
        when(() => mockOfflineOperations.updateChallengeProgress(
          'challenge-1',
        ...
        verify(() => mockOfflineOperations.updateChallengeProgress(
          'challenge-1',
          progressIncrement: 25,
          uid: testUserId,
        )).called(1);
      ...
      test('throws ChallengeNotFoundException when challenge does not exist', () async {
        // Arrange
        final mockQuery = MockQueryBuilder();
        when(() => mockCollection.filter()).thenReturn(mockQuery);
        when(() => mockQuery.challengeIdEqualTo('nonexistent')).thenReturn(mockQuery);
      ...
      test('throws ChallengeExpiredException when challenge is expired', () async {
        // Arrange
        final now = DateTime.now();
        ...
        final mockQuery = MockQueryBuilder();
        when(() => mockCollection.filter()).thenReturn(mockQuery);
        when(() => mockQuery.challengeIdEqualTo('expired-challenge')).thenReturn(mockQuery);
        when(() => mockQuery.findFirst()).thenAnswer((_) async => expiredChallenge);
      ...
      test('awards XP when challenge is completed', () async {
        // Arrange
        final now = DateTime.now();
        final nearCompletionChallenge = LocalChallenge()
        ...
        final mockQuery = MockQueryBuilder();
        when(() => mockCollection.filter()).thenReturn(mockQuery);
        when(() => mockQuery.challengeIdEqualTo('near-completion')).thenReturn(mockQuery);
        when(() => mockQuery.findFirst()).thenAnswer((_) async => nearCompletionChallenge);
        ...
        when(() => mockOfflineOperations.updateUserXP(
          testUserId,
          xpGained: 200,
        ...
        final result = await service.updateChallengeProgress(
          challengeId: 'near-completion',
          progressIncrement: 15, // This will complete the challenge (90 + 15 = 105, clamped to 100)
        );

        // Assert
        expect(result.completed, isTrue);
        expect(result.xpAwarded, equals(200));
        verify(() => mockOfflineOperations.updateUserXP(
          testUserId,
          xpGained: 200,
          reason: 'challenge_completion:near-completion',
        )).called(1);
