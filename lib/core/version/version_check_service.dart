...
class VersionCheckService {
  ...
  Future<VersionCheckResult> checkVersion() async {
    try {
      ...
      final minSupportedVersion = _remoteConfig.getString('min_supported_version');
      ...
      final isSupported = _compareVersions(currentVersion, minSupportedVersion) >= 0;
      final isRecommended = recommendedVersion.isEmpty || _compareVersions(currentVersion, recommendedVersion) >= 0;
  ...
  int _compareVersions(String current, String target) {
    ...
    final targetParts = target.split('.').map(int.tryParse).whereType<int>().toList();
...
sealed class VersionCheckResult {
  ...
  factory VersionCheckResult.supported(String currentVersion) = VersionSupported;
