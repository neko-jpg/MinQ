...
class StartupOptimizationService {
  ...
  StartupMetrics getStartupMetrics() {
    ...
    final timeToFirstFrame = _firstFrameTime != null && _appStartTime != null
  ...
  Future<List<String>> getStartupRecommendations() async {
    ...
    if (metrics.startupDuration.inMilliseconds > 3000) {
      recommendations.add('Startup time is slow (${metrics.startupDuration.inMilliseconds}ms). Consider enabling preload optimization.');
    }
    
    // Analyze time to first frame
    if (metrics.timeToFirstFrame.inMilliseconds > 1000) {
      recommendations.add('Time to first frame is slow (${metrics.timeToFirstFrame.inMilliseconds}ms). Consider reducing initial UI complexity.');
    }
    
    // Check preload settings
    if (!settings.preloadDataEnabled) {
      recommendations.add('Enable data preloading to improve startup performance.');
    }
    ...
    if (settings.coldStartCount > 10) {
      recommendations.add('Frequent cold starts detected. Consider implementing background refresh.');
    }
  ...
  Future<void> _recordStartupMetrics() async {
    try {
      ...
      if (_appStartTime != null) {
        await prefs.setInt(_startupTimeKey, _appStartTime!.millisecondsSinceEpoch);
      }
    } catch (e) {
