...
class PerformanceSettingsService {
  ...
  static const String _imageOptimizationEnabledKey = 'image_optimization_enabled';
  ...
  final StreamController<PerformanceSettings> _settingsController = StreamController.broadcast();
  
  static final PerformanceSettingsService _instance = PerformanceSettingsService._internal();
  ...
  Future<PerformanceSettings> getSettings() async {
    ...
    _cachedSettings = PerformanceSettings(
      ...
      imageOptimizationEnabled: prefs.getBool(_imageOptimizationEnabledKey) ?? true,
      lazyLoadingEnabled: prefs.getBool(_lazyLoadingEnabledKey) ?? true,
      performanceMode: PerformanceMode.values[prefs.getInt(_performanceModeKey) ?? 1], // Default to balanced
      frameRateLimit: prefs.getInt(_frameRateLimitKey) ?? 60,
      memoryOptimizationEnabled: prefs.getBool(_memoryOptimizationKey) ?? true,
      batteryOptimizationEnabled: prefs.getBool(_batteryOptimizationKey) ?? true,
      networkOptimizationEnabled: prefs.getBool(_networkOptimizationKey) ?? true,
    );
  ...
  Future<void> updateSettings(PerformanceSettings settings) async {
    ...
    await Future.wait([
      ...
      prefs.setBool(_particleEffectsEnabledKey, settings.particleEffectsEnabled),
      prefs.setBool(_heavyAnimationsEnabledKey, settings.heavyAnimationsEnabled),
      prefs.setBool(_imageOptimizationEnabledKey, settings.imageOptimizationEnabled),
      ...
      prefs.setBool(_batteryOptimizationKey, settings.batteryOptimizationEnabled),
      prefs.setBool(_networkOptimizationKey, settings.networkOptimizationEnabled),
    ]);
  ...
  Future<void> setParticleEffectsEnabled(bool enabled) async {
    final currentSettings = await getSettings();
    await updateSettings(currentSettings.copyWith(particleEffectsEnabled: enabled));
  }
  
  /// Set heavy animations enabled/disabled
  Future<void> setHeavyAnimationsEnabled(bool enabled) async {
    final currentSettings = await getSettings();
    await updateSettings(currentSettings.copyWith(heavyAnimationsEnabled: enabled));
  }
  ...
  Future<List<PerformanceRecommendation>> getRecommendations() async {
    ...
    if (deviceInfo.availableMemory < 2048) { // Less than 2GB RAM
      recommendations.add(PerformanceRecommendation(
        type: RecommendationType.memory,
        title: 'Enable Memory Optimization',
        description: 'Your device has limited memory. Enable memory optimization for better performance.',
        action: () => updateSettings(settings.copyWith(memoryOptimizationEnabled: true)),
        priority: RecommendationPriority.high,
      ));
    }
    
    // Animation recommendations
    if (deviceInfo.cpuCores < 4) { // Older/slower CPU
      recommendations.add(PerformanceRecommendation(
        type: RecommendationType.animation,
        title: 'Disable Heavy Animations',
        description: 'Disable heavy animations to improve performance on your device.',
        action: () => updateSettings(settings.copyWith(heavyAnimationsEnabled: false)),
        priority: RecommendationPriority.medium,
      ));
    }
    
    // Battery recommendations
    if (deviceInfo.batteryLevel < 0.2) { // Low battery
      recommendations.add(PerformanceRecommendation(
        type: RecommendationType.battery,
        title: 'Enable Battery Optimization',
        description: 'Your battery is low. Enable battery optimization to extend usage time.',
        action: () => updateSettings(settings.copyWith(batteryOptimizationEnabled: true)),
        priority: RecommendationPriority.high,
      ));
    }
    ...
    if (deviceInfo.isOnSlowNetwork) {
      recommendations.add(PerformanceRecommendation(
        ...
        description: 'Slow network detected. Enable network optimization for better experience.',
        action: () => updateSettings(settings.copyWith(networkOptimizationEnabled: true)),
        priority: RecommendationPriority.medium,
      ));
    }
    ...
    if (deviceInfo.maxRefreshRate < 60) {
      recommendations.add(PerformanceRecommendation(
        ...
        action: () => updateSettings(settings.copyWith(frameRateLimit: deviceInfo.maxRefreshRate)),
  ...
  Future<void> autoOptimize() async {
    ...
    debugPrint('Auto-optimized settings for device performance score: ${deviceInfo.performanceScore}');
  }
...
class PerformanceSettings {
  ...
  PerformanceSettings copyWith({
    ...
    return PerformanceSettings(
      ...
      heavyAnimationsEnabled: heavyAnimationsEnabled ?? this.heavyAnimationsEnabled,
      imageOptimizationEnabled: imageOptimizationEnabled ?? this.imageOptimizationEnabled,
      lazyLoadingEnabled: lazyLoadingEnabled ?? this.lazyLoadingEnabled,
      performanceMode: performanceMode ?? this.performanceMode,
      frameRateLimit: frameRateLimit ?? this.frameRateLimit,
      memoryOptimizationEnabled: memoryOptimizationEnabled ?? this.memoryOptimizationEnabled,
      batteryOptimizationEnabled: batteryOptimizationEnabled ?? this.batteryOptimizationEnabled,
      networkOptimizationEnabled: networkOptimizationEnabled ?? this.networkOptimizationEnabled,
    );
  }
}
