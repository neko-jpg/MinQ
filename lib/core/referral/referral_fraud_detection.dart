...
class ReferralFraudDetection {
  ...
  Future<ReferralValidationResult> validateReferral({
    ...
    try {
      ...
      if (referrerId == newUserId) {
        AppLogger.warning('Self-referral detected: referrer_id=$referrerId');
      ...
      if (!referrerValid) {
        AppLogger.warning('Referrer account too new: referrer_id=$referrerId');
        return ReferralValidationResult(
          isValid: false,
      ...
      if (dailyCount >= _maxReferralsPerDay) {
        AppLogger.warning('Daily referral limit exceeded: referrer_id=$referrerId, count=$dailyCount');
        return ReferralValidationResult(
          isValid: false,
      ...
      if (ipAddress != null) {
        ...
        if (ipCount >= _maxReferralsPerIP) {
          AppLogger.warning('IP referral limit exceeded: ip_address=$ipAddress, count=$ipCount');
          return ReferralValidationResult(
            isValid: false,
      ...
      if (recentCount >= 3) {
        AppLogger.warning('Suspicious referral pattern detected: referrer_id=$referrerId, count=$recentCount');
        return ReferralValidationResult(
          isValid: false,
      ...
      if (deviceId != null) {
        ...
        if (deviceUsed) {
          AppLogger.warning('Device already used for referral: device_id=$deviceId');
          return ReferralValidationResult(
            isValid: false,
      ...
      if (isBlacklisted) {
        AppLogger.warning('Blacklisted user attempted referral: referrer_id=$referrerId');
        return ReferralValidationResult(
          isValid: false,
      ...
      AppLogger.error('Referral validation failed', error: e, stackTrace: stack);
      return ReferralValidationResult(
        isValid: false,
  ...
  Future<void> addToBlacklist({
    ...
    try {
      ...
      AppLogger.info('User added to blacklist: user_id=$userId, reason=$reason');
    } catch (e, stack) {
      AppLogger.error('Failed to add user to blacklist', error: e, stackTrace: stack);
      rethrow;
    }
  }
  ...
  Future<void> removeFromBlacklist(String userId) async {
    try {
      ...
      AppLogger.error('Failed to remove user from blacklist', error: e, stackTrace: stack);
      rethrow;
    }
  }
  ...
  Future<void> recordSuspiciousActivity({
    ...
    try {
      ...
      AppLogger.warning('Suspicious activity recorded: user_id=$userId, reason=${reason.name}');
    } catch (e, stack) {
      AppLogger.error('Failed to record suspicious activity', error: e, stackTrace: stack);
    }
  }
  ...
  Future<FraudStats> getFraudStats({
    ...
    try {
      ...
      AppLogger.error('Failed to get fraud stats', error: e, stackTrace: stack);
      return FraudStats(
