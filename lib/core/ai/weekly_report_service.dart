...
class WeeklyReportService {
  ...
  static WeeklyReportService get instance => _instance ??= WeeklyReportService._();
  ...
  Future<WeeklyReport> generateReport({
    ...
    try {
      ...
      // レポート生成
      final report = await _generateDetailedReport(weeklyData, analysis, start, end);
  ...
  /// 週次データの収集
  Future<WeeklyData> _collectWeeklyData(String userId, DateTime start, DateTime end) async {
    ...
    return WeeklyData(
      ...
      averageMood: moods.isEmpty ? 3.0 : moods.map((m) => m.score).reduce((a, b) => a + b) / moods.length,
    );
  }
  ...
  Future<WeeklyAnalysis> _performAIAnalysis(WeeklyData data) async {
    ...
    // インサイト生成
    final insights = await _generateInsights(data, basicStats, trends, patterns);
...
''';
      ...
      return TrendAnalysis(
        ...
        description: trendAnalysis.isNotEmpty ? trendAnalysis : _getFallbackTrendDescription(data),
        confidence: 0.8,
  ...
  BehaviorPattern? _analyzeWeekdayPattern(WeeklyData data) {
    ...
    final maxDay = weekdayCompletions.entries
        .reduce((a, b) => a.value > b.value ? a : b);
    
    final minDay = weekdayCompletions.entries
        .reduce((a, b) => a.value < b.value ? a : b);
    
    if (maxDay.value - minDay.value >= 2) {
      return BehaviorPattern(
        type: PatternType.weekday,
        description: '${_getWeekdayName(maxDay.key)}に最も活発で、${_getWeekdayName(minDay.key)}は控えめです',
  ...
  BehaviorPattern? _analyzeTimePattern(WeeklyData data) {
    ...
    final maxHour = hourCompletions.entries
        .reduce((a, b) => a.value > b.value ? a : b);
    ...
    return BehaviorPattern(
      ...
      description: '${timeSlot}の時間帯に最も活発です',
  ...
  BehaviorPattern? _analyzeMoodPattern(WeeklyData data) {
    ...
    if (moodCompletionCorrelation.abs() > 0.3) {
      return BehaviorPattern(
        ...
        description: moodCompletionCorrelation > 0 
            ? '気分が良い日ほど習慣を継続できています'
            : '気分に関係なく習慣を継続できています',
        strength: moodCompletionCorrelation.abs(),
        recommendation: moodCompletionCorrelation > 0
  ...
  BehaviorPattern? _analyzeCategoryPattern(WeeklyData data) {
    ...
    final maxCategory = categoryCompletions.entries
        .reduce((a, b) => a.value > b.value ? a : b);
...
''';
      ...
      return PredictionAnalysis(
        ...
        aiPrediction: predictionText.isNotEmpty ? predictionText : '安定した継続が期待できます',
      );
    } catch (e) {
  ...
  Future<List<WeeklyInsight>> _generateInsights(
    ...
    if (stats.completionRate > 0.8) {
      insights.add(WeeklyInsight(
        ...
        title: '素晴らしい継続力！',
        description: '今週は${(stats.completionRate * 100).toStringAsFixed(0)}%の高い完了率を達成しました。',
        actionable: true,
      ...
      insights.add(WeeklyInsight(
        ...
        title: '継続のコツを見つけましょう',
        description: '今週の完了率は${(stats.completionRate * 100).toStringAsFixed(0)}%でした。',
        actionable: true,
    ...
    if (data.averageMood > 4.0) {
      insights.add(WeeklyInsight(
        ...
        title: '気分が絶好調！',
        description: '今週の平均気分は${data.averageMood.toStringAsFixed(1)}と高い水準でした。',
        actionable: true,
...
''';
  ...
  Future<WeeklyReport> _generateDetailedReport(
    ...
    // 祝福メッセージの生成
    final celebrationMessage = await _generateCelebrationMessage(data, analysis);
  ...
  /// 次週の目標設定
  List<WeeklyGoal> _generateNextWeekGoals(WeeklyData data, WeeklyAnalysis analysis) {
    ...
    goals.add(WeeklyGoal(
      ...
      description: '今週より${((targetCompletionRate - analysis.basicStats.completionRate) * 100).toStringAsFixed(0)}%向上を目標にします',
  ...
  /// 祝福メッセージの生成
  Future<String> _generateCelebrationMessage(WeeklyData data, WeeklyAnalysis analysis) async {
    try {
...
''';
  ...
  Future<List<HabitInfo>> _getHabitsForPeriod(String userId, DateTime start, DateTime end) async {
    // サンプルデータ
    return [
  ...
  Future<List<CompletionInfo>> _getCompletionsForPeriod(String userId, DateTime start, DateTime end) async {
    // サンプルデータ
  ...
  Future<List<MoodInfo>> _getMoodsForPeriod(String userId, DateTime start, DateTime end) async {
    // サンプルデータ
  ...
  Future<List<StreakInfo>> _getStreaksForPeriod(String userId, DateTime start, DateTime end) async {
    // サンプルデータ
    return [
  ...
  Future<void> _sendReportNotification(String userId, WeeklyReport report) async {
    // 通知送信の実装
  ...
  /// レポート履歴の取得
  Future<List<WeeklyReport>> getReportHistory(String userId, {int limit = 10}) async {
    // サンプルデータ
  ...
  /// 内部用のレポート生成メソッド
  Future<WeeklyReport> _generateWeeklyReport(String userId, DateTime startDate, DateTime endDate) async {
