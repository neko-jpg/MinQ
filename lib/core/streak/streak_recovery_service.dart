...
class StreakRecoveryService {
  ...
  Future<bool> purchaseRecoveryTicket({
    ...
    try {
      ...
      AppLogger().error(
        'Failed to purchase recovery ticket', e, stack,
      );
      return false;
    }
  }
  ...
  Future<bool> earnTicketByWatchingAd({required String userId}) async {
    try {
      ...
      AppLogger().error(
        'Failed to earn ticket by ad', e, stack,
      );
      return false;
    }
  }
  ...
  Future<int> getRecoveryTicketCount(String userId) async {
    try {
      ...
      AppLogger().error(
        'Failed to get recovery ticket count', e, stack,
      );
      return 0;
    }
  }
  ...
  Future<bool> canRecover({
    ...
    try {
      ...
      AppLogger().error(
        'Failed to check recovery availability', e, stack,
      );
      return false;
    }
  }
