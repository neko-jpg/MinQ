...
class BehaviorLearningService {
  ...
  BehaviorLearningService({required SharedPreferences prefs}) : _prefs = prefs;
  ...
  Future<void> recordNotificationOpened(
    ...
    data['notifications_opened'] =
        (data['notifications_opened'] as int? ?? 0) + 1;
  ...
  Future<void> _updateOptimalTimingAnalysis(
    ...
    final optimalHours =
        hourlyEngagementRates.entries
            .where(
              (entry) =>
                  entry.value >= averageHourlyEngagement && entry.value > 0.1,
            )
    ...
    final optimalDaysOfWeek =
        dailyEngagementRates.entries
            .where(
              (entry) =>
                  entry.value >= averageDailyEngagement && entry.value > 0.1,
            )
            .map((entry) => int.parse(entry.key))
            .toList()
          ..sort();
  ...
  /// 学習データをインポート
  Future<void> importLearningData(
    String userId,
    Map<String, dynamic> data,
  ) async {
    try {
  ...
  Future<void> cleanup() async {
    ...
    final keys = _prefs.getKeys().where(
      (key) =>
          key.startsWith(_behaviorDataKey) ||
          key.startsWith(_timingAnalysisKey),
    );
