...
class MicroCommitmentService {
  ...
  Future<void> createMicroQuest({
    ...
    String?
    originalQuestId, // Optional: link to the original quest it's a fallback for
  }) async {
    ...
    try {
      ...
      debugPrint("Creating micro-quest '$title' for user $userId.");
    } catch (e) {
      debugPrint('Error creating micro-quest: $e');
    }
  }
  ...
  Future<String?> suggestExpansion({
    ...
    try {
      ...
      final daysDifference =
          completionDates.first.difference(completionDates.last).inDays;
      ...
      debugPrint('Error suggesting expansion for $microQuestName: $e');
    }
    return null;
  }
