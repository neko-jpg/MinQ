...
class OfflineChallengeService {
  ...
  final StreamController<List<LocalChallenge>> _completedChallengesController =
      StreamController<List<LocalChallenge>>.broadcast();
  final StreamController<Map<String, ChallengeProgressData>>
  _progressController =
      StreamController<Map<String, ChallengeProgressData>>.broadcast();

  Stream<List<LocalChallenge>> get activeChallengesStream =>
      _activeChallengesController.stream;
  Stream<List<LocalChallenge>> get completedChallengesStream =>
      _completedChallengesController.stream;
  Stream<Map<String, ChallengeProgressData>> get progressStream =>
  ...
  Future<List<LocalChallenge>> getActiveChallenges() async {
    ...
    return await _isar.localChallenges
        ...
        .startDateLessThan(now, include: true)
        .endDateGreaterThan(now, include: true)
        .sortByStartDateDesc()
        .findAll();
  }
  ...
  Future<List<LocalChallenge>> getCompletedChallenges() async {
    return await _isar.localChallenges
        .filter()
        .progressGreaterThan(100, include: true) // Assuming 100% completion
        .or()
  ...
  Future<ChallengeProgressData?> getChallengeProgress(
    String challengeId,
  ) async {
    final challenge =
  ...
  Future<ChallengeProgressData> updateChallengeProgress({
    ...
    final newProgress = (challenge.progress + progressIncrement).clamp(
      0,
  ...
  Future<void> joinChallenge(String challengeId) async {
    ...
    if (!challenge.participants.contains(_userId)) {
      ...
      MinqLogger.info(
        'Joined challenge offline',
        metadata: {'challengeId': challengeId, 'userId': _userId},
      );
  ...
  Future<ChallengeStats> getUserChallengeStats() async {
    ...
    final completed =
        allChallenges.where((c) => c.progress >= c.targetValue).length;
    final active =
        allChallenges
            .where((c) => c.isActive && c.progress < c.targetValue)
            .length;
    ...
    return ChallengeStats(
      ...
      completionRate:
          allChallenges.isEmpty ? 0.0 : completed / allChallenges.length,
    );
  }
  ...
  Future<void> syncChallenges() async {
    if (_networkService.isOffline) {
      MinqLogger.warn('Cannot sync challenges while offline');
      return;
    }
  ...
  Future<void> _awardChallengeCompletionXP(LocalChallenge challenge) async {
    try {
      ...
      MinqLogger.info(
        'Challenge completion XP awarded',
        metadata: {
          'challengeId': challenge.challengeId,
      ...
      MinqLogger.error(
        'Failed to award challenge completion XP',
        exception: e,
        stackTrace: stackTrace,
      );
    }
...
class ChallengeProgressData {
  ...
  double get progressPercentage =>
      targetValue > 0 ? progress / targetValue : 0.0;
...
final offlineChallengeServiceProvider =
    Provider.family<OfflineChallengeService?, String>((ref, userId) {
...
final activeChallengesStreamProvider =
    StreamProvider.family<List<LocalChallenge>, String>((ref, userId) {
      final service = ref.watch(offlineChallengeServiceProvider(userId));
      return service?.activeChallengesStream ?? Stream.value([]);
    });

final completedChallengesStreamProvider =
    StreamProvider.family<List<LocalChallenge>, String>((ref, userId) {
      final service = ref.watch(offlineChallengeServiceProvider(userId));
...
final challengeProgressStreamProvider =
    StreamProvider.family<Map<String, ChallengeProgressData>, String>((
      ref,
      userId,
    ) {
      final service = ref.watch(offlineChallengeServiceProvider(userId));
...
final offlineOperationsProvider = Provider<OfflineOperationsService?>(
