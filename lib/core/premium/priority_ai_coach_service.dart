...
class PriorityAiCoachService {
  ...
  Future<List<AiInsight>> _generateAdvancedInsights(Map<String, dynamic> userContext) async {
    final insights = <AiInsight>[];
    
    // Analyze user patterns
    final streak = userContext['currentStreak'] as int? ?? 0;
    final completionRate = userContext['weeklyCompletionRate'] as double? ?? 0.0;
    final preferredTimes = userContext['preferredCompletionTimes'] as List<int>? ?? [];
    
    if (streak > 7) {
      insights.add(AiInsight(
        id: 'streak_momentum',
        title: 'Strong Momentum',
        description: 'Your $streak-day streak shows excellent consistency. This is a great foundation for long-term habit formation.',
    ...
    if (completionRate < 0.6) {
      insights.add(const AiInsight(
        ...
        description: 'Your completion rate could be improved. Consider reducing quest difficulty or adjusting timing.',
    ...
    if (preferredTimes.isNotEmpty) {
      ...
      insights.add(AiInsight(
        ...
        description: 'You tend to be most successful completing quests around $mostCommonHour:00. Consider scheduling important quests during this time.',
  ...
  Future<List<PersonalizedTip>> _generatePersonalizedTips(Map<String, dynamic> userContext) async {
    final tips = <PersonalizedTip>[];
    
    final focusTags = userContext['focusTags'] as List<String>? ?? [];
    final strugglingAreas = userContext['strugglingAreas'] as List<String>? ?? [];
    
    if (focusTags.contains('health')) {
      tips.add(const PersonalizedTip(
        id: 'health_stacking',
        title: 'Health Habit Stacking',
        description: 'Try linking your health quests to existing routines, like doing stretches right after brushing your teeth.',
        category: 'health',
    ...
    if (strugglingAreas.contains('consistency')) {
      tips.add(const PersonalizedTip(
        ...
        title: 'Start Smaller',
        description: 'Break down challenging quests into 2-minute micro-habits to build consistency before increasing difficulty.',
    ...
    if (focusTags.contains('productivity')) {
      tips.add(const PersonalizedTip(
        ...
        description: 'Dedicate specific time blocks for your productivity quests to avoid decision fatigue.',
  ...
  Future<PredictiveAnalysis?> _generatePredictiveAnalysis(Map<String, dynamic> userContext) async {
    final streak = userContext['currentStreak'] as int? ?? 0;
    final completionRate = userContext['weeklyCompletionRate'] as double? ?? 0.0;
    ...
    if (streak > 0) {
      streakContinuationProbability = (completionRate * 0.7) + (streak > 7 ? 0.2 : 0.1);
    }
    
    // Predict goal achievement
    final goalAchievementProbability = completionRate * 0.8 + (streak > 0 ? 0.15 : 0.0);
    ...
    return PredictiveAnalysis(
      streakContinuationProbability: streakContinuationProbability.clamp(0.0, 1.0),
      goalAchievementProbability: goalAchievementProbability.clamp(0.0, 1.0),
      riskFactors: riskFactors,
      successFactors: successFactors,
      recommendedActions: _generateRecommendedActions(riskFactors, successFactors),
      confidenceLevel: 0.8,
    );
  }

  List<String> _generateRecommendedActions(List<String> riskFactors, List<String> successFactors) {
  ...
  List<QuickAction> _generateAdvancedQuickActions(Map<String, dynamic> userContext) {
  ...
  List<QuickAction> _generateBasicQuickActions(Map<String, dynamic> userContext) {
    return [
      QuickAction(
        id: 'create_quest',
        title: 'Create Quest',
  ...
  List<String> _generateFollowUpQuestions(Map<String, dynamic> userContext) {
    ...
    final streak = userContext['currentStreak'] as int? ?? 0;
    if (streak > 0) {
      questions.add('What has been the key to maintaining your current streak?');
    ...
    questions.add('Which habit would have the biggest positive impact on your life?');
  ...
  String _buildAdvancedMessage(
    ...
    message += 'Based on your patterns, I have some personalized recommendations for you. ';
    message += 'How can I help you build on your momentum today?';
    
    return message;
  }

  String _buildBasicMessage(String userMessage, Map<String, dynamic> userContext) {
  ...
  Future<AiUsageStats> getUsageStats() async {
    final interactions = await _getStoredInteractions();
    final priorityInteractions = interactions.where((i) => i['isPriorityUser'] == true).toList();
    
    final avgResponseTime = interactions.isNotEmpty
        ? interactions.map((i) => i['responseTime'] as int).reduce((a, b) => a + b) / interactions.length
        : 0.0;
    
    final avgPriorityResponseTime = priorityInteractions.isNotEmpty
        ? priorityInteractions.map((i) => i['responseTime'] as int).reduce((a, b) => a + b) / priorityInteractions.length
        : 0.0;
    ...
    return AiUsageStats(
      ...
      averagePriorityResponseTime: Duration(milliseconds: avgPriorityResponseTime.round()),
      lastInteractionAt: interactions.isNotEmpty
