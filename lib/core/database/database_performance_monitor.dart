...
class DatabasePerformanceMonitor {
  ...
  static DatabasePerformanceMonitor get instance => _instance ??= DatabasePerformanceMonitor._();
  ...
  void _checkOperationPerformance(String operationType, Duration duration) {
    if (duration > _verySlowOperationThreshold) {
      _addAlert(PerformanceAlert(
        ...
        message: '$operationType took ${duration.inMilliseconds}ms (very slow)',
    ...
    for (final entry in _operationTimes.entries) {
      ...
      if (stats.averageDuration > _slowOperationThreshold) {
        _addAlert(PerformanceAlert(
          ...
          message: '$operationType average time: ${stats.averageDuration.inMilliseconds}ms',
          operationType: operationType,
      ...
      if (count > 1000) { // More than 1000 operations in monitoring period
        _addAlert(PerformanceAlert(
...
class PerformanceReport {
  ...
  String getSummary() {
    ...
    for (final entry in operationStatistics.entries) {
      ...
      buffer.writeln('  $op: $count operations, ${stats.averageDuration.inMilliseconds}ms avg');
    }
