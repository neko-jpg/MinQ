...
class InsightsEngine {
  ...
  Future<List<AnalyticsInsight>> generateAllInsights() async {
    ...
    // 優先度でソート
    insights.sort(
      (a, b) => _getPriorityScore(
        b.priority,
      ).compareTo(_getPriorityScore(a.priority)),
    );
  ...
  Future<List<AnalyticsInsight>> _generateContinuityInsights() async {
    ...
    final continuityRate = await _behaviorAnalysisService
        .analyzeHabitContinuityRate(startDate: startDate, endDate: endDate);
    ...
    if (continuityRate > 0.8) {
      insights.add(
        AnalyticsInsight(
          ...
          title: '素晴らしい継続率！',
          description:
              '過去30日間の習慣継続率が${(continuityRate * 100).toStringAsFixed(1)}%と非常に高い水準を維持しています。この調子で続けましょう！',
          actionItems: const [
            ActionItem(
              title: '進捗を共有',
          ...
          metadata: {'continuity_rate': continuityRate, 'period_days': 30},
      ...
      insights.add(
        AnalyticsInsight(
          ...
          title: '継続率の改善が必要です',
          description:
              '過去30日間の習慣継続率が${(continuityRate * 100).toStringAsFixed(1)}%と低下しています。小さな変更から始めてみましょう。',
          actionItems: const [
            ActionItem(
              title: 'クエストを簡単にする',
  ...
  Future<List<AnalyticsInsight>> _generateFailurePatternInsights() async {
    ...
    final failurePatterns =
        await _behaviorAnalysisService.analyzeFailurePatterns();

    for (final pattern in failurePatterns.take(3)) {
      // 上位3つのパターン
      if (pattern.confidence > 0.6) {
        ...
        insights.add(
          AnalyticsInsight(
            id:
                'failure_pattern_${pattern.type.name}_${DateTime.now().millisecondsSinceEpoch}',
  ...
  Future<List<AnalyticsInsight>> _generateOptimizationInsights() async {
    ...
    final bestTimePattern = timePatterns.fold<TimePattern?>(null, (
      best,
      current,
    ) {
    ...
    if (bestTimePattern != null && bestTimePattern.successRate > 0.8) {
      insights.add(
        AnalyticsInsight(
          ...
          title: '最適な時間帯を発見！',
          description:
              '${bestTimePattern.hour}時台の成功率が${(bestTimePattern.successRate * 100).toStringAsFixed(1)}%と最も高いです。この時間帯により多くのクエストをスケジュールしてみませんか？',
          actionItems: [
            ActionItem(
              title: 'スケジュールを調整',
    ...
    // 曜日最適化インサイト
    final dayPatterns =
        await _behaviorAnalysisService.analyzeDayOfWeekPatterns();
    final bestDayPattern = dayPatterns.fold<DayOfWeekPattern?>(null, (
      best,
      current,
    ) {
      if (best == null || current.successRate > best.successRate) {
    ...
    if (bestDayPattern != null && bestDayPattern.successRate > 0.8) {
      insights.add(
        AnalyticsInsight(
          ...
          description:
              '${bestDayPattern.dayName}曜日の成功率が${(bestDayPattern.successRate * 100).toStringAsFixed(1)}%と最も高いです。この曜日のパターンを他の曜日にも応用してみましょう。',
          actionItems: [
            ActionItem(
              title: 'パターンを分析',
  ...
  Future<List<AnalyticsInsight>> _generateAchievementInsights() async {
    ...
    final seasonalPatterns =
        await _behaviorAnalysisService.analyzeSeasonalPatterns();
    ...
    if (currentSeasonPattern.successRate > 0.7) {
      insights.add(
        AnalyticsInsight(
          ...
          description:
              '${currentSeason.displayName}の成功率が${(currentSeasonPattern.successRate * 100).toStringAsFixed(1)}%と好調です。人気カテゴリ: ${currentSeasonPattern.popularCategories.join(', ')}',
          actionItems: const [
            ActionItem(
              title: '成果を記録',
