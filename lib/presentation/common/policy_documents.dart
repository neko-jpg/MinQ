...
final Map<PolicyDocumentId, PolicyDocument>
policyDocuments = <PolicyDocumentId, PolicyDocument>{
    ...
    sections: <PolicySection>[
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                'MinQは、匿名のペアと共に1日3タップで習慣化を促すモバイルアプリです。利用者は匿名プロフィールを作成し、習慣クエストの記録、Pairとのハイタッチを通じた励ましを行えます。',
            en:
                'MinQ is a mobile application that helps users build habits with an anonymous partner in as little as three taps per day. Users create an anonymous profile to log quests and exchange supportive high-fives with their pair.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                '本サービスは13歳以上の方を対象としています。未成年の方は保護者の同意を得た上でご利用ください。13歳未満の方は登録や利用はできません。',
            en:
                'This service is intended for individuals aged 13 and older. Minors should obtain consent from a guardian before using the app. Users under the age of 13 are not permitted to register or use MinQ.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                'Pairとのやり取りは互いを尊重する姿勢で行い、誹謗中傷やハラスメント、個人情報の共有を禁止します。不適切な利用が報告された場合はアカウント制限、停止を行います。',
            en:
                'Interact with your pair respectfully. Harassment, abusive language, and sharing personally identifiable information are prohibited. Accounts may be limited or suspended if inappropriate behaviour is reported.',
          ),
          PolicyParagraph(
            ja: '共有する写真は本人や第三者の個人情報が写り込まないようにしてください。違反が見つかった場合は削除や審査対象となります。',
            en:
                'Ensure that proof photos do not expose personal information about yourself or others. Violations may lead to removal or moderation review.',
          ),
          PolicyParagraph(
            ja:
                '特に、未成年者の安全を確保するため、成人ユーザーが未成年者に対して不適切な関係を求める言動や、個人情報を聞き出す行為を固く禁じます。保護者の方は、未成年のお子様の利用状況を適切に監督する責任を負います。',
            en:
                'To ensure the safety of minors, adult users are strictly prohibited from soliciting inappropriate relationships or personal information from minors. Guardians are responsible for appropriately supervising their minor\'s use of the service.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                'Pairとのやり取りで不快な体験があった場合は、アプリ内のメニューから通報・ブロックが可能です。通報は24時間以内に一次対応し、安全チームが状況を確認します。',
            en:
                'If you encounter uncomfortable behaviour, use the in-app menu to report or block your pair. Reports receive an initial review within 24 hours by the safety team.',
          ),
        ],
      ),
      PolicySection(
    ...
    sections: <PolicySection>[
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                '匿名プロフィール、アプリ内のQuest達成ログ、Pairとのハイタッチ履歴、通知設定、任意で連携したGoogleアカウントIDを取得します。写真証跡は端末内でEXIFを除去し、ハッシュ化されたファイル名で保存されます。',
            en:
                'We collect anonymous profile details, quest completion logs, pair high-five history, notification preferences, and optional linked Google account IDs. Photo proofs have EXIF data removed on-device and are stored using hashed filenames.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                '習慣継続の可視化、通知配信、マッチング精度向上、アプリ改善のための分析に利用します。個人が特定できる形で第三者と共有することはありません。',
            en:
                'Data is used to visualise progress, deliver notifications, improve matching quality, and analyse app performance. We do not share data with third parties in a personally identifiable form.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                '通信はすべてTLSで暗号化され、保存データは暗号化されたストレージに保持されます。アカウント削除申請から7日間の復元猶予期間が経過した後、データを完全に削除します。この期間内に再度ログインすると、削除リクエストはキャンセルされます。',
            en:
                'All network traffic is encrypted via TLS and stored data resides in encrypted storage. After a 7-day grace period following an account deletion request, all data is permanently removed. Logging in again within this period will cancel the deletion request.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                'アプリ内からデータの閲覧、エクスポート、削除申請が可能です。お問い合わせは support@minq.app までお寄せください。',
            en:
                'You can review, export, or request deletion of your data from within the app. Contact support@minq.app for assistance.',
          ),
        ],
      ),
    ],
      ...
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ...
            en:
                'Within 24 hours of receiving a report, our team reviews the situation and may take interim actions such as temporarily pausing the pair.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ...
            en:
                'We review logs and partner interactions and request additional details if required. Findings and resolutions are shared with the reporter via email.',
          ),
        ],
      ),
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja:
                'レポート内容を分析し、テンプレート改善やレート制限などの機能改善を行います。安全に関するインサイトはチーム内の週次レビューで共有します。',
            en:
                'We analyse reports to inform improvements such as template updates or rate limiting. Safety insights are reviewed in weekly team meetings.',
          ),
        ],
      ),
    ],
  ...
  PolicyDocumentId.licenses: const PolicyDocument(
    ...
    sections: <PolicySection>[
      PolicySection(
        ...
        paragraphs: <PolicyParagraph>[
          PolicyParagraph(
            ja: 'アプリアイコンおよびイラストは社内デザインチームによるオリジナル制作物です。第三者提供素材は使用していません。',
            en:
