...
const List<String> _quickReplyTemplates = <String>[
  '縺翫▽縺九ｌ縺輔∪・・,
  '莉翫°繧牙叙繧顔ｵ・∩縺ｾ縺呻ｼ・,
  '莉頑律縺ｯ縺薙ｌ縺ｧ縺翫＠縺ｾ縺・↓縺励∪縺吶・,
  '蜉ｩ縺代※縺上ｌ縺ｦ縺ゅｊ縺後→縺・ｼ・,
];

const List<String> _stampOptions = <String>['聡', '櫨', '潮', '剏', '笨ｨ'];
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                if (!isMe) ...[
                  const SizedBox(width: 8),
                  _ReactionButton(emoji: '総', onTap: () => addReaction('総')),
                  _ReactionButton(emoji: '聡', onTap: () => addReaction('聡')),
                  _ReactionButton(emoji: '櫨', onTap: () => addReaction('櫨')),
                ],
              ],
            ),
          ),
  ...
  Future<void> _sendMessage() async {
    ...
    if (moderationResult.isBlocked) {
      FeedbackMessenger.showErrorSnackBar(
        context,
        moderationResult.details ?? '荳埼←蛻・↑蜀・ｮｹ縺悟性縺ｾ繧後※縺・∪縺・,
      );
      return;
    }
    ...
    if (moderationResult.isFlagged) {
      ...
      final shouldSend = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('蜀・ｮｹ縺ｮ遒ｺ隱・),
          content: Text(moderationResult.details ?? '縺薙・蜀・ｮｹ繧帝∽ｿ｡縺励※繧ゅｈ繧阪＠縺・〒縺吶°・・),
          actions: [
            TextButton(
