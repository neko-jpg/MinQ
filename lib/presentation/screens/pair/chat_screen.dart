  ...
  Future<void> _showReportDialog(BuildContext context, WidgetRef ref) async {
    ...
    if (confirmed == true && reasonController.text.isNotEmpty) {
      await ref
          ...
          ?.reportUser(currentUserId, buddyId, reasonController.text);
      if (context.mounted) {
    ...
    return PopupMenuButton<String>(
      ...
      itemBuilder:
          (BuildContext context) => <PopupMenuEntry<String>>[
            PopupMenuItem<String>(value: 'block', child: Text(l10n.blockUser)),
...
class _MessageInputBarState extends ConsumerState<_MessageInputBar> {
  ...
  Future<void> _sendQuickText(String text) async {
    ...
    try {
      ...
      if (mounted) {
        FeedbackMessenger.showErrorSnackBar(context, l10n.messageSentFailed);
      }
    } finally {
      if (mounted) {
        setState(() => _isSending = false);
                  ...
                  if (_isUploading)
                    ...
                    Builder(
                      builder: (context) {
                        ...
                        final reduceMotion = AnimationSystem.shouldReduceMotion(
                          context,
                        );
