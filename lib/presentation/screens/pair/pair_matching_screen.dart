...
class _PairMatchingScreenState extends ConsumerState<PairMatchingScreen> with TickerProviderStateMixin {
  ...
  Future<void> _subscribeToNotifications() async {
    ...
    if (settings.authorizationStatus != AuthorizationStatus.authorized) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('通知が許可されませんでした。')));
      return;
    }
    ...
    if (token != null && uid != null && repo != null) {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('通知を登録しました！')));
      context.pop();
    } else {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('通知の登録に失敗しました。')));
    }
  }
  ...
  Widget _buildSearchingUI(BuildContext context, MinqTheme tokens) {
    return Column(
      ...
      children: [
            ...
            children: [
              Container(
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: tokens.brandPrimary.withOpacity(0.2), width: 4),
                ),
              ),
              RotationTransition(
                turns: _controller,
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    border: Border.all(color: tokens.brandPrimary.withOpacity(0.4), width: 4),
                  ),
                ),
              ),
        ...
        Text(
          'バディを探しています…',
          style: tokens.titleLarge.copyWith(color: tokens.textPrimary, fontWeight: FontWeight.bold),
        ),
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                _buildInfoRow(tokens, '匿名ID', 'Buddy#${_foundPairId?.substring(0, 4) ?? '????'}'),
                Divider(color: tokens.border, height: 32),
                _buildInfoRow(tokens, '年齢帯', '18-24'), // TODO: Get real data
                Divider(color: tokens.border, height: 32),
                _buildInfoRow(tokens, '目標カテゴリ', 'Fitness'), // TODO: Get real data
              ],
            ),
          ),
          const Spacer(flex: 3),
  ...
  Widget _buildNoMatchUI(BuildContext context, MinqTheme tokens) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          const Spacer(),
          Icon(Icons.sentiment_dissatisfied, color: tokens.brandPrimary, size: 72),
          const SizedBox(height: 24),
          Text(
            '現在、条件に合う\n候補が見つかりませんでした',
            textAlign: TextAlign.center,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '🎉 バディ成立！ 🎉',
            style: tokens.displaySmall.copyWith(color: tokens.textPrimary, fontWeight: FontWeight.bold),
          ),
          const SizedBox(height: 16),
          Text(
          ...
          MinqPrimaryButton(
            label: '共通クエストを始める',
            onPressed: () async => ref.read(navigationUseCaseProvider).goHome(), // Navigate to home or quest screen
          ),
          const SizedBox(height: 16),
          MinqSecondaryButton(
            label: '後で開始する',
  ...
  Widget _buildInfoRow(MinqTheme tokens, String label, String value) {
    return Row(
      ...
      children: [
        ...
        Text(value, style: tokens.bodyMedium.copyWith(color: tokens.textPrimary, fontWeight: FontWeight.bold)),
      ],
    );
  }
      ...
      body: Center(
        child: AnimatedSwitcher(
          ...
          child: switch (_status) {
            ...
            PairMatchingStatus.matchFound => _buildMatchFoundUI(context, tokens),
