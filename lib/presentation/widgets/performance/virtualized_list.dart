...
class _VirtualizedListState<T> extends State<VirtualizedList<T>> {
  ...
  int _getIndexAtOffset(double offset) {
    if (widget.itemHeight != null) {
      return (offset / widget.itemHeight!).floor().clamp(0, widget.items.length - 1);
    }
...
class _HeightMeasureWidget extends SingleChildRenderObjectWidget {
  ...
  @override
  void updateRenderObject(BuildContext context, _HeightMeasureRenderObject renderObject) {
...
class QuestVirtualizedList extends StatelessWidget {
  ...
  final Widget Function(BuildContext context, dynamic quest, int index) itemBuilder;
