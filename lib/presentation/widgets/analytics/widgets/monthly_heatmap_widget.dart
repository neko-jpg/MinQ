...
class MonthlyHeatmapWidget extends ConsumerWidget {
  ...
  const MonthlyHeatmapWidget({super.key, required this.config});
      ...
      child: Center(
        child: Text(
          ...
          style: TextStyle(
            ...
            fontWeight:
                completionCount > 0 ? FontWeight.bold : FontWeight.normal,
          ),
        ),
      ),
    );
  ...
  Widget _buildLegend(BuildContext context, Map<DateTime, int> heatmapData) {
    final maxCompletions = heatmapData.values.fold<int>(
      0,
  ...
  Widget _buildEmptyState(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          const Icon(Icons.calendar_view_month, size: 32, color: Colors.grey),
          const SizedBox(height: 8),
          Text(
            'データなし',
            style: Theme.of(
