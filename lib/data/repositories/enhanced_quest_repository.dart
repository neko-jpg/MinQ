...
class EnhancedQuestRepository extends QuestRepository with DatabasePerformanceTracking {
  ...
  Future<List<Quest>> getTemplateQuests() {
    return trackOperation('quest_get_templates', () => super.getTemplateQuests());
  }
  
  @override
  Future<List<Quest>> getQuestsForOwner(String owner) {
    return trackOperation('quest_get_by_owner', () => super.getQuestsForOwner(owner));
  }
  ...
  Future<void> seedInitialQuests() {
    return trackOperation('quest_seed_initial', () => super.seedInitialQuests());
  }
  ...
  Future<int> cleanupUnusedQuests({
    ...
    return trackOperation('quest_cleanup', () async {
      ...
      return await isar.writeTxn(() async {
        final unusedQuests = await isar.quests
            .filter()
  ...
  Future<QuestRepositoryStats> getStats() {
    return trackOperation('quest_get_stats', () async {
      ...
      final templateCount = await isar.quests.filter().isTemplateEqualTo(true).count();
